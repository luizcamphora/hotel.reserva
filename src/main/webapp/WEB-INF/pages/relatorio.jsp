...
<script type="text/javascript">
    ...
    function drawChart() {
    	...
    	$.ajax({
            url : "/reservas/relatorio/geral/dados",
            data : param,
            dataType : "JSON",
            type : "GET",
            ...
            success : function(response) {
            	var dataArray = [
            		['Dados', 'Total de Reservas', 'Aguardando Aprovação', 'Ativa', 'Finalizada', 'Cancelada - Hotel', 'Cancelada - Cliente'],
            	];
            	
            	for (var i = 0; i < response.length; i++) {
                    var row = [response[i].mes, response[i].reservas, response[i].aguardandoAprovacao, response[i].ativa, response[i].finalizada, response[i].canceladaHotel, response[i].canceladaCliente];
                    dataArray.push(row);
                }
        		...
        		var data = google.visualization.arrayToDataTable(dataArray);
         		...
         		var options = {
        			chart: {
                    	title: 'Relatório de reservas dos dias ' + document.getElementById('dataInicioFormat').value + " até o dia "+ document.getElementById('dataFimFormat').value,
                    	subtitle: 'Relatório com o quantitativo das reservas',
                  	},
